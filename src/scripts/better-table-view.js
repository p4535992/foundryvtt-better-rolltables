...
export class BetterRT {
  ...
  static async enhanceRollTableView(rollTableConfig, html, rollTable) {
    const tableClassName = rollTable.cssClass,
      ...
      selectedTableType =
        tableEntity.getFlag(MODULE.ns, BRTCONFIG.TABLE_TYPE_KEY) ||
        BRTCONFIG.TABLE_TYPE_NONE,
    ...
    if (
      selectedTableType === BRTCONFIG.TABLE_TYPE_BETTER ||
      selectedTableType === BRTCONFIG.TABLE_TYPE_LOOT
    ) {
      BetterRT.ShowFormulaField(
        tableViewClass,
        tableEntity,
        rollTable.editable
      );
    ...
    if (selectedTableType === BRTCONFIG.TABLE_TYPE_LOOT) {
      ...
      const displayRollElement =
        inputElements.namedItem('displayRoll').parentElement;
      ...
      headerElement.querySelectorAll('.toggler').forEach(async (e) => {
        e.addEventListener('click', async (e) => {
          const toggleIconElement =
            e.currentTarget.querySelector('.toggleicon');
          e.currentTarget.nextElementSibling.classList.toggle('brt-hidden');
          ['fa-expand-alt', 'fa-compress-alt'].map((c) =>
            toggleIconElement.classList.toggle(c)
          );
        });
      });
    }
  ...
  static ShowFormulaField(tableViewClass, tableEntity, editable) {
    const tableResultsHTML =
      tableViewClass.getElementsByClassName('table-result');
    let index = 0;
    for (const resultHTML of tableResultsHTML) {
      ...
      if (resultId) {
        ...
        const detailsHTML =
          resultHTML.getElementsByClassName('result-details')[0];
          ...
          if (tableText.getAttribute('type') === 'text') {
            ...
            if (tableText.classList.contains('result-target')) {
              formulaInput.value =
                getProperty(
                  tableResult,
                  `flags.${MODULE.ns}.${BRTCONFIG.RESULTS_FORMULA_KEY}.formula`
                ) || '';
  ...
  static Tags(element, dataCarrier, listOfTags) {
    let DOMParent = document.querySelector(element),
      ...
      arrayOfList =
        listOfTags ||
        DOMHiddenInput.value.split(',').filter((tag) => tag !== '');
    ...
    function DOMRender() {
      ...
      arrayOfList.forEach(function (currentValue, index) {
        ...
        li.querySelector('a').addEventListener('click', async (e) => {
          ...
          DOMHiddenInput.value = DOMHiddenInput.value
            .replace(replace, '')
            .trim(',');
