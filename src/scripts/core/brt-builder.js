    ...
    while (amount > 0) {
      ...
      for (const entry of draw.results) {
        const formulaAmount =
          getProperty(
            entry,
            `flags.${BRTCONFIG.NAMESPACE}.${BRTCONFIG.RESULTS_FORMULA_KEY}.formula`
          ) || '';
        ...
        if (
          ...
          if (
            entityInCompendium !== undefined &&
            entityInCompendium.documentName === 'RollTable'
          ) {
        ...
        if (innerTable) {
          const innerResults = await this.rollManyOnTable(
            entryAmount,
            innerTable,
            { _depth: _depth + 1 }
          );
