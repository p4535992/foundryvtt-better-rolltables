...
export class LootCreator {
  ...
  async createActor(table, overrideName = undefined) {
    const actorName =
      overrideName || table.getFlag(MODULE.ns, BRTCONFIG.ACTOR_NAME_KEY);
    ...
    const lootSheet = game.settings.get(
      MODULE.ns,
      BRTCONFIG.LOOT_SHEET_TO_USE_KEY
    );
  ...
  async addCurrenciesToActor() {
    ...
    for (const key in lootCurrency) {
      if (Object.getOwnPropertyDescriptor(currencyData, key)) {
        const amount =
          Number(currencyData[key].value || 0) + Number(lootCurrency[key]);
  ...
  async _createLootItem(item, actor, stackSame = true, customLimit = 0) {
    const newItemData = await this.buildItemData(item),
      ...
      originalItem = embeddedItems.find(
        (i) =>
          i.name === newItemData.name &&
          itemPrice === getProperty(i, BRTCONFIG.PRICE_PROPERTY_PATH)
      );
    ...
    if (originalItem && stackSame) {
      ...
      const newItemQty =
          getProperty(newItemData, BRTCONFIG.QUANTITY_PROPERTY_PATH) || 1,
        originalQty =
          getProperty(originalItem, BRTCONFIG.QUANTITY_PROPERTY_PATH) || 1,
        updateItem = { _id: originalItem.id },
        newQty = this._handleLimitedQuantity(
          newItemQty,
          originalQty,
          customLimit
        );
  ...
  async buildItemData(item) {
    ...
    if (!itemData) return;
    itemData = await this.lootManipulator.preItemCreationDataManipulation(
      itemData
    );
    return itemData;
  }
  ...
  async addCurrenciesToToken(token, isTokenActor = false) {
    ...
    for (const key in currencyDataInitial) {
      const amount =
        Number(currencyData[key] || 0) + Number(lootCurrency[key] || 0);
  ...
  async addItemsToToken(
    ...
    for (const item of this.betterResults) {
      ...
      const newItem = await this._createLootItem(
        item,
        targetActor,
        stackSame,
        customLimit
